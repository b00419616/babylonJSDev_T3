import { Scene, ArcRotateCamera, Vector3, DirectionalLight, MeshBuilder, Color3, ShadowGenerator, StandardMaterial, Texture } from "@babylonjs/core";
...
function createArcRotateCamera(scene) {
    let camAlpha = (5 * Math.PI) / 4, camBeta = Math.PI / 3, camDist = 12, camTarget = new Vector3(0, 0, 0);
    let camera = new ArcRotateCamera("camera1", camAlpha, camBeta, camDist, camTarget, scene);
    camera.attachControl(true);
    return camera;
}
export default function createScene1(engine) {
    ...
    that.scene.registerBeforeRender(() => {
        if (that.sphere) {
            that.sphere.rotation.y += 0.01;
        }
        if (that.torus) {
            that.torus.rotation.y -= 0.01;
        }
    });
